...
<template>
  ...
  >
    <div class="item-group" :key="el.pid" v-for="(el, index) in realValue">
            ...
            <el-col :span="20">
              ...
              <div v-else>
                <div v-if="el.isEdit">
                  ...
                  <el-row>
                    <el-col :span="7">price:</el-col>
                    <el-col :span="17"
                      ><el-input v-model="el.price" placeholder="" size="mini"
                    /></el-col>
                  </el-row>
                  <el-row :gutter="10">
                    <el-col :span="12">是否需要图标:</el-col>
      ...
      <sell class="item-sub" :list="el.content" />
    </div>
  </draggable>
</template>
...
export default {
  ...
  watch: {
    realValue: {
      //深度监听，可监听到对象、数组的变化
      handler() {
        sessionStorage.setItem("Snpid", this.npid);
      },
      deep: true, //true 深度监听
    },
  },
  created() {
    let npid = sessionStorage.getItem("Snpid");
    if (npid != null) {
      this.npid = npid;
    }
  },
